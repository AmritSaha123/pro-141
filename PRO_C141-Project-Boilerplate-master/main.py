...
def get_article():
    articledata = assign_val()
    return jsonify({"data":articledata,"status":"success"})
...
def unliked_article():
    articledata = assign_val()
    not_liked_articles.append(articledata)
...
def liked_article():
    articledata = assign_val()
    liked_articles.append(articledata)
    all_articles = all_articles.reset_index()
    return jsonify({"status":"success"})
